...
BACKEND_POD=$(kubectl get pods -n shop-system -l app.kubernetes.io/component=backend -o jsonpath='{.items[0].metadata.name}')
...
ELASTICSEARCH_POD=$(kubectl get pods -n shop-system -l app.kubernetes.io/component=elasticsearch -o jsonpath='{.items[0].metadata.name}')
